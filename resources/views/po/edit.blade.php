...
<div class="card">
    ...
    <div class="card-body">
        <div class="form-group">
            <a class="btn btn-info" href="{{ route('purchase_order.show', $po->id) }}">
                Back
            </a>
        ...
        <form autocomplete="off" method="POST" action="{{ route("purchase_order.update", [$po->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf

            <div class="form-group">
                <label class="required" for="po_num">PO #</label>
                <input class="form-control" type="text" name="po_num" id="po_num" value="{{ old('po_num', $po->po_num) }}" required>
                <span class="help-block"></span>
            </div>

            <div class="form-group">
                <label class="required" for="po">PO</label>
                <input class="form-control" type="text" name="po" id="po" value="{{ old('po', $po->po) }}" required>
                <span class="help-block"></span>
            </div>

            <div class="form-group">
                <label class="required" for="issuance_num">Issuance #</label>
                <input class="form-control" type="text" name="issuance_num" id="issuance_num" value="{{ old('issuance_num', $po->issuance_num) }}" required>
                <span class="help-block"></span>
            </div>

            <div class="form-group">
                <label class="required" for="po_date">Purchase Order Date</label>
                <input class="form-control" type="date" name="po_date" id="po_date" value="{{ old('po_date', $po->po_date) }}" required>
                <span class="help-block"></span>
            </div>

            <div class="form-group">
                <label class="required" for="supplier">Supplier</label>
                <input class="form-control" type="text" name="supplier" id="supplier" value="{{ old('supplier', $po->supplier) }}" required>
                <span class="help-block"></span>
            </div>

            <div class="form-group">
                <label class="required" for="released_by">Released By</label>
                <input class="form-control" type="text" name="released_by" id="released_by" value="{{ old('released_by', $po->released_by) }}" required>
                <span class="help-block"></span>
            </div>
            <div class="form-group">
                <label class="required" for="received_by">Received By</label>
                <input class="form-control" type="text" name="received_by" id="received_by" value="{{ old('received_by', $po->received_by) }}" required>
                <span class="help-block"></span>
            </div>

            <div class="form-group">
                <label for="endorsed_to">Endorsed To</label>
                <input class="form-control" type="text" name="endorsed_to" value="{{ old('endorsed_to', $po->endorsed_to) }}" id="endorsed_to">
                <span class="help-block"></span>
            </div>
...
<div class="card">
    <div class="card-header">
        ...
        <button type="button" class="add-modal btn-xs btn btn-primary" data-toggle="modal" data-target="#addModal">
            Add Item
        </button>
    </div>
                ...
                <tbody>
                    @foreach ($items as $item)
                        <tr data-entry-id="">
                            ...
                            <td>{{ $item->amount }}</td>
                            <td>
                                <button type="button" class="edit-modal btn btn-xs btn-info" data-toggle="modal" data-target="#editModal" data-id="{{ $item->id }}" data-qty="{{ $item->qty }}" data-unit="{{ $item->unit }}" data-description="{{ $item->description }}" data-price="{{ $item->price }}">
                                    Edit
                                </button>

                                <form action="{{ route('po_item.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete?');" style="display: inline-block;">
        ...
        <div class="modal-body"><!--inputs located-->
            <table class="table table-bordered">
                ...
                <tbody class="item-row">
                    <form autocomplete="off" method="POST" action="{{ route('po_item.store') }}" enctype="multipart/form-data">
                        <input type="hidden" name="id" value="{{ $po->id }}" class="form-control">
                        @csrf
                    <tr>
                        <td>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body"><!--inputs located-->
            <table class="table table-bordered">
                ...
                <tbody>
                    @if(isset($item))
                        {!! Form::model($item, ['route' => ['po_item.update', 'edit-item-id'], 'method' => 'PUT']) !!}
                        ...
                        <tr>
                            ...
                            <td>
                                {!! Form::text('price', null, ['id' => 'edit-item-price', 'class' => "form-control"]) !!}  
                            </td> 
                        </tr>  
